...
export const useStore = create<StoreState>((set, get) => ({
    ...
    set((state) => {
      ...
      const updatedTasks = [...state.repairTasks, ...newTasks].map(task => {
        if (task.status === 'in-progress') {
          const timeSpent = differenceInMinutes(newTime, task.startTime);
  ...
  completeTask: (taskId: string) => {
    ...
    set((state) => {
      ...
      // Only allow completion if user is manager or the assigned adjuster
      if (user.role !== 'MANAGER' && user.adjusterId !== task.adjusterId) return state;
      ...
      const updatedMachines = state.machines.map(machine =>
        machine.id === task.machineId
          ? {
              ...machine,
              status: 'operational',
              nextPredictedFailure: addHours(state.simulation.currentTime, machine.mttf),
            }
          : machine
      );
      ...
      const updatedAdjusters = state.adjusters.map(adjuster =>
        adjuster.id === task.adjusterId
          ? { ...adjuster, status: 'available', currentTask: null }
          : adjuster
      );
      ...
      return {
        machines: updatedMachines,
        adjusters: updatedAdjusters,
        repairTasks: state.repairTasks.filter(t => t.id !== taskId),
      };
    });
  },
  ...
  addMachine: (machine) => {
    ...
    set((state) => ({
      machines: [...state.machines, { ...machine, id: generateId() }],
    }));
  },
  ...
  addAdjuster: (adjuster) => {
    ...
    set((state) => ({
      adjusters: [...state.adjusters, { ...adjuster, id: generateId() }],
    }));
  },
}));